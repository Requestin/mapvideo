...
- Убрать `grid-template-columns: 1fr 320px` у `editor-page__body`; превью во всю ширину
- Левое меню «Элементы» — выпадающее по кнопке (кнопка видна всегда, список — по клику)
- Правая панель — overlay, рендерится **только** когда выбран элемент
- Esc и клик по пустому месту карты (не по элементу) — закрывают обе панели
- Нижний toolbar +50%: `--layout-toolbar-height` 56 → 84, кнопки крупнее
- Превью строго 16:9 сохраняется (панели оверлейные, не сдвигают канвас)
