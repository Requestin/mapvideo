...
Пройти все сценарии вручную перед деплоем:

**Авторизация:**
- [ ] Логин с верными данными → попадаем на редактор
- [ ] Логин с неверными данными → ошибка
- [ ] Закрыть браузер и открыть снова → не просит логиниться
- [ ] Выйти → редиректит на /login
- [ ] Открыть /admin под admin → работает
- [ ] Открыть /admin под обычным юзером → редирект на /

**Точки:**
- [ ] Добавить точку через геосаджест
- [ ] Добавить точку без выбора подсказки (ручной ввод)
- [ ] Добавить 3+ точки → карта автомасштабируется
- [ ] Перетащить точку → маршруты перестраиваются
- [ ] Перетащить подпись независимо от точки
- [ ] Все 4 анимации работают с live preview
- [ ] Удалить точку → маршруты удаляются, подтверждение показывается
- [ ] Сбросить местоположение точки/подписи

**Маршруты:**
- [ ] Построить маршрут между двумя точками
- [ ] Построить маршрут в произвольное место (стрелка)
- [ ] Escape отменяет построение
- [ ] Маршрут по дороге (OSRM) для автомобиля
- [ ] Дуга для самолёта
- [ ] Иконки движутся и поворачиваются по направлению
- [ ] Удалить точку → связанный маршрут удаляется

**Настройки видео:**
- [ ] Переключение темы → карта меняется сразу
- [ ] Ползунок дыхания → анимация показывается пока открыто меню
- [ ] Закрыть меню → дыхание останавливается
- [ ] Длительность вне диапазона 3-60 → валидация

**Рендер:**
- [ ] Нажать "Сохранить видео" → интерфейс блокируется
- [ ] Прогресс-бар обновляется каждые 2 сек
- [ ] После завершения → файл скачивается автоматически
- [ ] Закрыть браузер во время рендера и открыть снова → видим прогресс-бар
- [ ] Готовое видео появляется в "Моя история"
- [ ] Повторное скачивание из истории работает
