...
- 14.1 Миграция БД: `users.custom_colors TEXT[] NOT NULL DEFAULT '{}'`
(массив hex-строк в формате `#rrggbb`).
- 14.2 Backend: `GET /api/users/me/colors`, `PUT /api/users/me/colors`
(лимит 10 штук, регэксп `#rrggbb`, дедуп, требует сессию + CSRF на PUT).
Тесты в `backend/tests/misc.test.ts`.
- 14.3 Frontend: `api/user-colors.ts`, провайдер `UserColorsContext`
(single fetch на старте редактора; optimistic PUT на add/remove).
- 14.4 Новый `ColorField` с поповером: 10 пресетов + до 10 кастомных
