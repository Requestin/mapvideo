---
name: vibesec-skill
description: This skill helps write secure web applications. Use it when working on web features, auth, input handling, or when a user requests a security scan/audit.
---
...
Token misconfigurations can lead to full authentication bypass and session hijacking.

### Vulnerabilities

| Vulnerability | Prevention |
|---------------|------------|
| `alg: none` attack (JWT) | Always verify algorithm server-side, reject `none` |
| Algorithm confusion (JWT) | Explicitly specify expected algorithm, never derive from token |
| Weak token secrets | Use 256+ bit cryptographically random secrets |
| Missing expiration | Always set and validate expiration |
| Token in localStorage | Prefer httpOnly, Secure, SameSite cookies; avoid localStorage for auth tokens |


### Secure Implementation (JWT example)
...
- [ ] Expiration always set and validated
- [ ] Session/token values stored in httpOnly cookies (not localStorage/sessionStorage)
- [ ] Logout/session revocation invalidates server-side session state
- [ ] If using refresh tokens, rotation is implemented (old refresh token invalidated on use)

> Note: this section includes JWT-specific examples, but secure cookie-session (opaque token + server-side session store) is also valid and often preferred.
