...
- Фаза 8 доведена: интеграция редактора (`editor-workspace` + `BottomToolbar`), API `frontend/src/api/render.ts`, e2e-логика без реального Chromium в Jest (`jest.mock('../src/render/render-video')`), `VIDEOS_DIR` в `tests/global-setup.ts` → `backend/tests/tmp-videos/`.
- Следующий шаг по плану SPEC — **task9.md** (полировка и деплой); при сбоях headless-рендера в Docker проверить `FRONTEND_URL`, `PUPPETEER_EXECUTABLE_PATH`, `waitUntil: load` в `render-video.ts`.