...
- [x] Zoom-dependent scale для точек и подписей (меньше — при zoom-out, не налезают друг на друга)
- [x] Починить показ иконок `explosion.png` / `fire_loop` / `earthquake.png` (асинхронная загрузка текстур Pixi)
- [x] Пульсация самой точки (мигающая анимация) — `scale 1 ⇔ 1.15`, синхронно с ореолом, относительно размера из ползунка
- [x] Hover-highlight: при наведении на элемент на карте подсвечиваются и сам элемент, и строка в `ElementsList`
- [x] Клик по строке в `ElementsList` — элемент моргает 2–3 раза (alpha pulse). Если вне кадра — предварительно `resetView()`
